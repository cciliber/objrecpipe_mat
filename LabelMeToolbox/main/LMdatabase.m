        ...
        if isfield(v.annotation, 'object')
            ...
            for m = 1:Nobjects
                ...
                if isfield(v.annotation.object(m), 'name') 
                    name = lower(v.annotation.object(m).name);
                    if ~isempty(name) && ~isempty(strtrim(name))
                        v.annotation.object(m).name = strtrim(name);
                    else
                        v.annotation.object(m).name = ''; % empty names can be a problem later, so lets put a space
                        v.annotation.object(m).deleted = '1'; % empty names can be a problem later, so lets put a space
                    end
                end
                
                % add id
                if isfield(v.annotation.object(m), 'id') && ~isempty(v.annotation.object(m).id)
                    v.annotation.object(m).id = str2num(v.annotation.object(m).id);
                else
                    v.annotation.object(m).id = m-1; % uses C convention for indices
                end
