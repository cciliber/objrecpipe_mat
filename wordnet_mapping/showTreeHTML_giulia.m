function showTreeHTML_giulia(tree, pagename)
% showTreeHTML(tree)
%
...
% tree.Node = is a cell array. Each element is the name associated
%    with each node.
...
% Header
page = {};
page = addline(page, '<html><head><title>iCubWorld tree</title><body>');
%page = addline(page, '<img src="http://labelme.csail.mit.edu/Icons/LabelMeNEWtight198x55.gif" height=55 alt=":LabelMe" />');
page = addline(page, sprintf('<b>+ <a href="http://wordnet.princeton.edu/" style="font-size:%dem;">WordNet</a></b><br>', 2*fsize));
...
%page = addline(page, sprintf('<div style="position:absolute; top:120px; left: 6px; background-color:yellow;font-size:%dem;">', fsize));
...
for n = 1:length(x)
    ...
    % if tree.counts(n)>threshold
        page = addline(page, texthtml(x(n), y(n), chain, fsize));
...
function line = texthtml(x,y,s, fsize)
...
line = [line sprintf('<a href="http://people.csail.mit.edu/torralba/research/LabelMe/js/LabelMeQueryObjectFast.cgi/?query=%s" title="%s" style="font-size:%dem;">', sq, s, fsize)];
...
if x2>x1
    ...
    line = [line sprintf('<img style="width:%5.1fem;height:%5.1fem;border:0px;" src="media/lineright_%d.gif">',  x2-x1, y2-y1, bestratio)];
    line = [line '</div>'];
else
    line = sprintf('<div style="position:absolute;top:%5.1fem;left:%5.1fem;">', y1, x2);
    line = [line sprintf('<img style="width:%5.1fem;height:%5.1fem;border:0px;" src="media/lineleft_%d.gif">', (x1-x2), (y2-y1), bestratio)];
    line = [line '</div>'];
end
