function fcn_class_exp_cat(machine, dataset_name, modality, task, feature_name)
...
loaded_list_train = load(fullfile(experiments_path, train_list_filename));
objlist_train = loaded_list_train.objlist_train;
loaded_list_test = load(fullfile(experiments_path, test_list_filename));
objlist_test = loaded_list_test.objlist_test;
loaded_list_cat = load(fullfile(experiments_path, cat_list_filename));
catlist = loaded_list_cat.catlist;

Nnuples = zeros(1, Ncat-1);
for no=1:Ncat-1
    Nnuples(no) = size(objlist_train{no},1);
end
...
for nc=2:Ncat
    ...
    for nu_cat=1:Nnuples(nc-1)
        ...
        for o=1:NobjPerCat
            ...
            y_path = [];
            labels_list = catlist{nc-1}{nu_cat};
            ...
            selected_folders_train = objlist_train{nc-1}{nu_cat,o};
            
            feat_train.load_feat(feat_rootpath, in_registry_path, feat_ext, selected_folders_train, out_registry_path);
            ...
            selected_folders_test = objlist_test{nc-1}{nu_cat,o};
            
            feat_test.load_feat(feat_rootpath, in_registry_path, feat_ext, selected_folders_test, out_registry_path);
            ...
            cell_output{nc-1}{nu_cat, o}.experiment = selected_folders_test;
