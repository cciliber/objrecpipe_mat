...
for idx_day_train=1:numel(data_day)  
    ...
    for idx_day_test=1:numel(data_day)
        ...
        if idx_day_test == idx_day_train
            ...
            W = tuple_paramsel(Xsel,I(Ysel,:),Xtr{idx_day_test},I(Ytr{idx_day_test},:),20);

            [~,Ypred] =  max(Xts{idx_day_test}*W,[],2);

            error_day(idx_day_train,idx_day_test)=trace_confusion(Yts{idx_day_test},Ypred);
        end
    end
...
model_all = gurls_train(Xall,Yall,'kernelfun','linear','nlambda',20,'nholdouts',5,'hoproportion',0.5,'partuning','ho','verbose',1);


for idx_day_test=1:numel(data_day)

    W = tuple_paramsel(Xall,I(Yall,:),Xtr{idx_day_test},I(Ytr{idx_day_test},:),20);

    [~,Ypred] =  max(Xts{idx_day_test}*W,[],2);

    error_day(end,idx_day_test)=trace_confusion(Yts{idx_day_test},Ypred);
