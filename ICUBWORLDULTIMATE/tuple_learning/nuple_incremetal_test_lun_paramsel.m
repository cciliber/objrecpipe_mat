...
%     %
%     
%     W = tuple_paramsel(X,Y,Xva,Yva,100);
%     
%     [~,Ypred] =  max(cell2mat(Xts_c)*W,[],2);
%     acc_nuples(idx_n)=trace_confusion(cell2mat(Yts_c),Ypred);     
%    
%     %
%     
%     %
...
% 
%         [~,Ypred] =  max(cell2mat(Xts_c)*W,[],2);
%         acc_nuples_tc(idx_t-1,idx_n)=trace_confusion(cell2mat(Yts_c),Ypred);     
% 
...
% 
% %         
% %         for idx_tt = 3:idx_t-1
% %             plot(nuples+(idx_tt-2)*min_n_tc,acc_nuples_tc(idx_tt-1,:));
% %         end
% 
%         plot(nuples(1:idx_n)+(idx_t-2)*min_n_tc-1,acc_nuples_tc(idx_t-1,1:idx_n));
%     
%         
%     end
%     
...
%     
%     for idx_t = 3:4
%         line([(idx_t-2)*min_n_tc-1 (idx_t-2)*min_n_tc-1],get(hax,'YLim'),'Color','k','LineStyle','--');
%     end
%     
%     pause(0.1);
% 
...
%%
acc_nuples_new = zeros(numel(nuples),1);
fig = figure;
for idx_n = 1:numel(nuples)
    ...
    for idx_t = 3:3
        ...
        Xo = cell2mat(Xtr_ct{2});
        Yo = cell2mat(Ytr_ct{2});
             
        X = cellfun(@(X)X(1:nuples(idx_n),:),Xtr_ct{idx_t},'UniformOutput',0);
        Y = cellfun(@(Y)Y(1:nuples(idx_n),:),Ytr_ct{idx_t},'UniformOutput',0);
        ...
        [~,Ypred] =  max(cell2mat(Xts_c)*W,[],2);
        acc_nuples_new(idx_n)=trace_confusion(cell2mat(Yts_c),Ypred);     

        plot_nuples = [nuples+(idx_t-2)*min_n_tc-1, nuples(1:idx_n)+(idx_t-1)*min_n_tc-1]; 
        %model_tc{idx_t-1}{idx_n} = gurls_train(X,Y,'kernelfun','linear','nlambda',20,'nholdouts',5,'hoproportion',0.8,'partuning','loo','verbose',verbose);

        
        plot_acc = [acc_nuples_tc(idx_t-1,:), acc_nuples_new(1:idx_n)'];
        % test on present
        %acc_nuples_tc(idx_t,idx_n)=trace_confusion(cell2mat(Yts),gurls_test(model_tc{idx_t-1}{idx_n},cell2mat(Xts)));    
...
%         end
        ...
        plot(plot_nuples,plot_acc);
    
        
    end
    
    plot(nuples+2*min_n_tc-1,acc_nuples_tc(3,1:numel(nuples)));
