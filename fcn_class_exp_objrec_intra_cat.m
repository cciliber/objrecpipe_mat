...
objlist_filename = 'objlist_objrec_intra_cat.mat';
...
Nnuples = zeros(1, NobjPerCat-1);
for no=1:NobjPerCat-1
...
cell_output = cell(NobjPerCat-1,1);
for no=1:NobjPerCat-1
    cell_output{no} = cell(Nnuples(no),Ncat);
end
...
for no=2:NobjPerCat
    ...
    for nu_obj=1:Nnuples(no-1)
        ...
        for c=1:Ncat
            ...
            feat_ext = '.mat';
            selected_folders = objlist{no-1}{nu_obj,c};
            out_registry_path = [];
            ...
            cell_output{no-1}{nu_obj, c}.accuracy = result.opt.perf.acc;
            cell_output{no-1}{nu_obj, c}.ypred_double = result.opt.pred;
            [~, yte_class] = max(yte, [], 2);
            cell_output{no-1}{nu_obj, c}.y = yte_class;
            cell_output{no-1}{nu_obj, c}.experiment = selected_folders;
            
            save(ys_path, 'cell_output');
            delete(result.opt.savefile);

            disp(['no = ' num2str(no) ' nu_obj = ' num2str(nu_obj) ' c = ' num2str(c)]);
