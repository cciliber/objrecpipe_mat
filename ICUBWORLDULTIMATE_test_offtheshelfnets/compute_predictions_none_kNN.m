...
obj_list = 1:NobjPerCat;
transf_list = 1:Ntransfs;
camera_list = [1 2];
...
for cc=cat_idx
    ...
    N = size(Xeven{opts.Cat(cat_names{cc})},1);
    Ypred{opts.Cat(cat_names{cc})} = zeros(N, 1);
    Nbatches = ceil(N/batch_size);

    for idx=1:Nbatches 
        Ypred{opts.Cat(cat_names{cc})}((((idx-1)*batch_size+1):min(idx*batch_size, N))) = kNNClassify_multiclass(cell2mat(Xodd), cell2mat(Yodd), k, Xeven{opts.Cat(cat_names{cc})}(((idx-1)*batch_size+1):min(idx*batch_size, N),:));
...
for cc=cat_idx 
    ...
    N = size(Xodd{opts.Cat(cat_names{cc})},1);
    Ypred{opts.Cat(cat_names{cc})} = zeros(N, 1);
    Nbatches = ceil(N/batch_size);

    for idx=1:Nbatches 
        Ypred{opts.Cat(cat_names{cc})}(((idx-1)*batch_size+1):min(idx*batch_size,N)) = kNNClassify_multiclass(cell2mat(Xeven), cell2mat(Yeven), k, Xodd{opts.Cat(cat_names{cc})}(((idx-1)*batch_size+1):min(idx*batch_size,N),:));
        disp([set_name ': ' num2str(cc) ' ' num2str(idx)]);
    end
end
save(fullfile(output_dir, ['YkNN_' set_name '.mat']), 'Ypred', '-v7.3');
