function fcn_class_exp_objrec_random_nuples(machine, dataset_name, modality, task, feature_name)
...
opt_path = fullfile(output_path, ['opt_' modality '_' feature_name]);
...
%objlist_filename = 'objlist.mat';
objlist_filename = 'objlist_30_ICRA15.mat';
...
Nnuples = zeros(1, Nobj-1);
for no=1:Nobj-1
    Nnuples(no) = length(objlist{no});
end
...
cell_output = cell(Nobj-1,1);
for no=1:Nobj-1
    cell_output{no} = cell(Nnuples(no),1);
end
...
try
    ...
    for no=1:Nobj-1
        ...
        for nu_idx=1:Nnuples(no)
            ...
            feat_ext = '.mat';
            selected_folders = objlist{no}{nu_idx};
            out_registry_path = [];
            ...
            cell_output{no}{nu_idx}.accuracy = result.opt.perf.acc;
            cell_output{no}{nu_idx}.ypred_double = result.opt.pred;
            [~, yte_class] = max(yte, [], 2);
            cell_output{no}{nu_idx}.y = yte_class;
            cell_output{no}{nu_idx}.experiment = selected_folders;
            
            save(ys_path, 'cell_output');
            delete(result.opt.savefile);
            
            disp(['no = ' num2str(no+1) ' nu_idx = ' num2str(nu_idx)]);
        end
    end
